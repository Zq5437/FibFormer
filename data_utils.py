...
class SERDataset:
    ...
    def __init__(self, features_data, num_classes = 4,
        ...
        # self.num_in_ch = 1
        # NOTE
        self.train_spec_data = self._spec_to_rgb_my(self.train_spec_data,"train")
        self.val_spec_data = self._spec_to_rgb_my(self.val_spec_data,"val")
        self.test_spec_data = self._spec_to_rgb_my(self.test_spec_data,"test")
        self.num_in_ch = 3
    ...
    def save_mfcc_pic(self):
        def process_and_save_mfcc(data, save_path_prefix):
            os.makedirs(save_path_prefix, exist_ok=True)

            def process_single_mfcc(index):
                try:
                    seg_mfcc = data["seg_mfcc"][index]
                    save_path_mfcc = os.path.join(save_path_prefix, f"{index}.png")

                    # 归一化处理
                    min_val, max_val = np.min(seg_mfcc), np.max(seg_mfcc)
                    seg_mfcc_normalized = (seg_mfcc - min_val) / (max_val - min_val)

                    # 保存为图片
                    plt.imsave(save_path_mfcc, seg_mfcc_normalized, cmap='jet')
                    
                    # 读取图片
                    img = plt.imread(save_path_mfcc)
                    img = img[:, :, :3]
                    
                    # 变为单通道
                    single_channel_img = np.dot(img, [0.2989, 0.5870, 0.1140])
                    
                    # 通过反归一化恢复到原始的数据范围，但是最大最小应该不可知
                    # single_channel_img = (single_channel_img - np.min(single_channel_img)) / (np.max(single_channel_img) - np.min(single_channel_img))
                    # single_channel_img = single_channel_img * (max_val - min_val) + min_val

                    # 更新原始数据
                    # 如果想要只保存图片，而不影响原来的值，只需要把下面这行给注释掉即可
                    data['seg_mfcc'][index] = np.array(single_channel_img, dtype=np.float32)
                except Exception as e:
                    print(f"Error processing MFCC index {index}: {e}")

            # 使用多线程加速处理，限制最大线程数以防止内存问题
            with ThreadPoolExecutor(max_workers=8) as executor:
                executor.map(process_single_mfcc, range(len(data['seg_mfcc'])))

        # 处理训练数据
        train_save_path_mfcc_prefix = os.path.join("/home/h666/Zq/2025ICASSP/savePic/mfcc", 'train')
        process_and_save_mfcc(self.train_data, train_save_path_mfcc_prefix)

        # 处理验证数据
        val_save_path_mfcc_prefix = os.path.join("/home/h666/Zq/2025ICASSP/savePic/mfcc", 'val')
        process_and_save_mfcc(self.val_data, val_save_path_mfcc_prefix)

        # 处理测试数据
        test_save_path_mfcc_prefix = os.path.join("/home/h666/Zq/2025ICASSP/savePic/mfcc", 'test')
        process_and_save_mfcc(self.test_data, test_save_path_mfcc_prefix)
    
    
    def _spec_to_rgb_my(self, data, kind):
        """
        Convert normalized spectrogram to pseudo-RGB image based on pyplot color map
        and apply AlexNet image pre-processing
        ...
        Input: data
                ...
                - data range [0.0, 1.0]
        """
        train_save_path_spec_prefix = os.path.join("/home/h666/Zq/2025ICASSP/savePic/spec", kind)
        os.makedirs(train_save_path_spec_prefix, exist_ok=True)
        ...
        def process_segment(index):
            try:
                save_path_spec = os.path.join(train_save_path_spec_prefix, f"{index}.png")
                
                # 方式1   直接用颜色映射计算保存图片后得到的图片的值
                # seg = np.clip(data[index], 0.0, 1.0)
                # seg_rgb = (cm(seg)[:, :, :3] * 255.0).astype(np.uint8)  # Apply colormap and convert to RG
                # img = Image.fromarray(seg_rgb, mode='RGB')  # Convert to image object
                # preprocessed_img = alexnet_preprocess(img)  # Apply preprocessing pipeline
                # # Optional: Save the image (only if needed for debugging or other purposes)
                # img.save(save_path_spec)
                # return preprocessed_img
                
                # 方式2   使用plt.imsave保存图片
                seg = np.clip(data[index], 0.0, 1.0)
                plt.imsave(save_path_spec, seg, cmap='jet')
                img = plt.imread(save_path_spec)
                img = img[:,:,:3]
                img = Image.fromarray(img, mode='RGB')
                preprocessed_img = alexnet_preprocess(img)
                return preprocessed_img
            
            
                # 如果只想保存图片，而不影响原来的数据，可以取消注释下面的一块代码
                # seg = np.clip(data[index], 0.0, 1.0)
                # plt.imsave(save_path_spec, seg, cmap='jet') # 不想保存图片的话，把这行注释掉就可以了
                # seg_rgb = (cm(seg)[:,:,:3]*255.0).astype(np.uint8)
                # img = Image.fromarray(seg_rgb, mode='RGB')
                # preprocessed_img = alexnet_preprocess(img)
                # return preprocessed_img
                ...
                print(f"Error processing spectrogram segment {index}: {e}")
                return None

        # Use ThreadPoolExecutor to process segments in parallel
        with ThreadPoolExecutor(max_workers=8) as executor:
            data_tensor = list(executor.map(process_segment, range(len(data))))

        # Filter out any None values if any segment processing failed
        data_tensor = [tensor for tensor in data_tensor if tensor is not None]
