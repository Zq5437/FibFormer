...
class Ser_Model(nn.Module):
    def __init__(self, ratio=None):
        ...
        self.base_model_feature_extractor = nn.Sequential(*list(self.base_model.children())[:-1])
        ...
        self.post_spec_dropout = nn.Dropout(p=0.1)
        self.post_spec_layer = nn.Linear(512 * 7 * 7, 128)
        
        self.post_mfcc_dropout = nn.Dropout(p=0.1)
        self.post_mfcc_layer = nn.Linear(512 * 7 * 7, 128)
        
        self.post_wav_dropout = nn.Dropout(p=0.1)
        self.post_wav_layer = nn.Linear(512 * 7 * 7, 128)
