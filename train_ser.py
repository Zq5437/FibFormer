...
def main(args):
    ...
    # Load dataset
    # args.features_file = "/home/h666/Zq/2025ICASSP/baseline/CA-MSER/IEMOCAP_multi.pkl"
    args.features_file = "/home/h666/Zq/2025ICASSP/baseline/CA-MSER/features_extraction/IEMOCAP_multi345200.pkl"
    ...
    # Train
    print("*" * 20, " current ratio: ", args.ratio, "*" * 20)
    print()
    train_stat = train(ser_dataset, params, save_label=args.save_label, ratio=args.ratio)
...
def train(dataset, params, save_label='default', ratio=None):
    torch.cuda.set_device("cuda:0")
    # NOTE mfcc
    dataset.save_mfcc_pic()
    print("mfcc pic saved !")
    # NOTE
    ...
    model = Ser_Model(ratio=ratio).to(device) 
    
    # 打印模型结果，因为太占格子了，所以先注释掉
    # print(model.eval())
    ...
    # 保存图片，没有使用多线程
    # # train
    # train_save_path_wav2vec_prefix = os.path.join("/home/h666/Zq/2025ICASSP/savePic/wav2vec", "train")
    # os.makedirs(train_save_path_wav2vec_prefix, exist_ok=True)
    # seg_audio_list = []
    # for i in range(len(dataset.train_data["seg_audio"])):
    #     wav2vec_save_path = os.path.join(train_save_path_wav2vec_prefix, str(i) + ".png")
    #     seg_audio = dataset.train_data["seg_audio"][i]
    #     wav2vec_feature = model.save_pic_wav2vec(seg_audio,wav2vec_save_path,device)
    #     seg_audio_list.append(wav2vec_feature)
    # seg_audio_list = [torch.tensor(arr) for arr in seg_audio_list]
    # dataset.train_data["seg_audio"] = torch.stack(seg_audio_list)
    
    # # test
    # test_save_path_wav2vec_prefix = os.path.join("/home/h666/Zq/2025ICASSP/savePic/wav2vec", "test")
    # os.makedirs(test_save_path_wav2vec_prefix, exist_ok=True)
    # seg_audio_list = []
    # for i in range(len(dataset.test_data["seg_audio"])):
    #     wav2vec_save_path = os.path.join(test_save_path_wav2vec_prefix, str(i) + ".png")
    #     seg_audio = dataset.test_data["seg_audio"][i]
    #     wav2vec_feature = model.save_pic_wav2vec(seg_audio,wav2vec_save_path,device)
    #     seg_audio_list.append(wav2vec_feature)
    # seg_audio_list = [torch.tensor(arr) for arr in seg_audio_list]
    # dataset.test_data["seg_audio"] = torch.stack(seg_audio_list)
    
    # # val
    # val_save_path_wav2vec_prefix = os.path.join("/home/h666/Zq/2025ICASSP/savePic/wav2vec", "val")
    # os.makedirs(val_save_path_wav2vec_prefix, exist_ok=True)
    # seg_audio_list = []
    # for i in range(len(dataset.val_data["seg_audio"])):
    #     wav2vec_save_path = os.path.join(val_save_path_wav2vec_prefix, str(i) + ".png")
    #     seg_audio = dataset.val_data["seg_audio"][i]
    #     wav2vec_feature = model.save_pic_wav2vec(seg_audio,wav2vec_save_path,device)
    #     seg_audio_list.append(wav2vec_feature)
    # seg_audio_list = [torch.tensor(arr) for arr in seg_audio_list]
    # dataset.val_data["seg_audio"] = torch.stack(seg_audio_list)
    # print("wav2vec pic saved !")
    
    
    # speed up version ---- 使用多线程，将图片保存并直接将图片数据赋值
    # def process_audio_segments(data, save_path_prefix, model, device):
    #     os.makedirs(save_path_prefix, exist_ok=True)
    #     seg_audio_list = []

    #     def process_single_segment(index):
    #         try:
    #             seg_audio = data["seg_audio"][index]
    #             save_path = os.path.join(save_path_prefix, f"{index}.png")
    #             wav2vec_feature = model.save_pic_wav2vec_speedup(seg_audio, save_path, device)
    #             if not torch.is_tensor(wav2vec_feature):
    #                 return torch.tensor(wav2vec_feature)
    #             else:
    ...
    #         seg_audio_list = list(executor.map(process_single_segment, range(len(data["seg_audio"]))))
        
    #     data["seg_audio"] = torch.stack(seg_audio_list)

    # # Train
    # train_save_path_wav2vec_prefix = os.path.join("/home/h666/Zq/2025ICASSP/savePic/wav2vec", "train")
    # process_audio_segments(dataset.train_data, train_save_path_wav2vec_prefix, model, device)

    # # Test
    # test_save_path_wav2vec_prefix = os.path.join("/home/h666/Zq/2025ICASSP/savePic/wav2vec", "test")
    # process_audio_segments(dataset.test_data, test_save_path_wav2vec_prefix, model, device)

    # # Val
    # val_save_path_wav2vec_prefix = os.path.join("/home/h666/Zq/2025ICASSP/savePic/wav2vec", "val")
    # process_audio_segments(dataset.val_data, val_save_path_wav2vec_prefix, model, device)
    ...
    for epoch in range(params['num_epochs']):
        ...
        with tqdm(train_loader) as td:
            for train_batch in td:
                ...
                #    y_true.append
                print("outputs: ", outputs['M'].shape)
                train_preds.append(f.log_softmax(outputs['M'], dim=1).cpu().detach().numpy())
                print("train_labels_batch: ", train_labels_batch.shape)
                print("train_preds: ", train_preds[0].shape)
                # Compute the loss, gradients, and upoutputs[date the parameters
                train_loss_ce = criterion_ce('M'], train_labels_batch)
